...
-- PlayerIndex.h
PlayerIndex = { One = 1, Two =2,Three =3,Four=4,Null=5 }
...
function Init(object)
	...
	--object.direction = Vector3(0.0,0.0,0.0);

    return "Content/Models/Cube.rtx";
end

function Input(object,input,currentIndex)
	...
	if(input.UseGamepad == true) then
		local playerIndex
		if(input:IsDPADUpButtonPress(currentIndex,playerIndex)) then
			dir = Vector3(dir.x,1,dir.z);
			y = speed
		end
...
function Update(object,dt)
	...
	targetRadians = MathHelper:AngleFromXY(object.direction.y,object.direction.x);

	if ( (targetRadians >= MathHelper.Infinity)(targetRadians <= -MathHelper.Infinity)) then
		targetRadians = 0.0;
	end

	radians = targetRadians;

	object:worldMatrix = Matrix:CreateRotationZ(radians) * Matrix:CreateTranslation(object:position);
end